...
- Replaced the small-allocation trim-eligibility average, previously a single global `AtomicU32` overwritten with a fresh raw batch average on every `(cpu, class)` pair scanned during a trim pass, with a per-size-class `EmaPredictor` (`TRIM_PREDICTOR`, `EMA_ALPHA = 0.25`). Each class now blends new observations into its own smoothed estimate instead of the last-scanned class clobbering the shared threshold used by every other class and CPU.
...
- Nudge a class's `TRIM_PREDICTOR` toward its ceiling when a trim pass finds nothing to scan for that class, so a quiet class doesn't get stuck at a stale low average and make the next block that appears there look instantly trim-eligible. Gated behind the new `trim-aggressively` Cargo feature, which skips the nudge and keeps trim eligibility tighter for workloads that want more aggressive reclamation.
- Added a real cross-class global average: `AVERAGE_BLOCK_TIMES` (read by the background trimmer's scheduling gate, `RSMallocStats`, and `debug_exit_printer`) is now computed once per `trim_small()` call as the mean of every trim-eligible class's current `TRIM_PREDICTOR` value, instead of holding whatever the last-processed class happened to leave behind.
...
- Switched `CURRENT_STAMP` and all lifetime/average-lifetime bookkeeping (`trim.rs`, buddy trim, `TRIM_PREDICTOR` seed and clamps) from raw milliseconds to 100ms ticks, quadrupling `AtomicU32`'s wraparound headroom. Every `CURRENT_STAMP` writer (`get_clock()`, preload bootstrap, Rust `RSMalloc::init()`) and every millisecond-scale clamp/threshold was rescaled to match (`/100` for constants that were already in milliseconds), and display/reporting paths (`debug_exit_printer`, `RSMallocStats::avg_small_life_ms`/`avg_buddy_life_ms`) multiply back by 100 so externally visible values remain true milliseconds.
- Fixed a bootstrap bug from the unit-conversion above: `AVERAGE_BLOCK_TIMES`/`BUDDY_AVERAGE_BLOCK_TIMES` kept their pre-conversion initial seed (`1000`), which the background trimmer's gate now read as 1000 *ticks* (100 real seconds) instead of the intended 1 second, silently delaying the first background trim pass by roughly 100x on every process start.
