...
pub(crate) unsafe fn print_report() {
    ...
    {
        use crate::{
            FILL_HIT_LATENCY_HIST, FILL_STEAL_LATENCY_HIST, LATENCY_BUCKETS, LOCAL_FILL_HITS,
        };

        fn percentile_cycles(
            hist: &[std::sync::atomic::AtomicUsize; crate::LATENCY_BUCKETS],
            total: usize,
            frac: f64,
        ) -> usize {
        ...
        for class in 0..SIZE_CLASSES.len() {
            let hist = &FILL_HIT_LATENCY_HIST[class];
            let total: usize = hist.iter().map(|b| b.load(Relaxed)).sum();
            let local_hits = LOCAL_FILL_HITS[class].load(Relaxed);
            if total == 0 && local_hits == 0 {
                continue;
            }

            let p50 = percentile_cycles(hist, total, 0.50);
            let p99 = percentile_cycles(hist, total, 0.99);
        ...
        section(
            &mut report,
            "fill() steal-path latency (cycles, log2-bucketed)",
        );
        line(&mut report, "  cls  size       samples     p50        p99");
        for class in 0..SIZE_CLASSES.len() {
            let hist = &FILL_STEAL_LATENCY_HIST[class];
            let total: usize = hist.iter().map(|b| b.load(Relaxed)).sum();
            if total == 0 {
                continue;
            }
